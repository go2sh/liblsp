...
public:
  ...
  virtual void error(const std::string &Message) const {
    Client->logMessage({lsp::MessageType::Error, Message});
  }

  /// Log an info message
  virtual void info(const std::string &Message) const {
    Client->logMessage({lsp::MessageType::Info, Message});
  }

  /// Log a warning message
  virtual void warning(const std::string &Message) const {
    Client->logMessage({lsp::MessageType::Warning, Message});
  }

  /// Log a generic message
  virtual void log(const std::string &Message) const {
    Client->logMessage({lsp::MessageType::Log, Message});
  }
};
} // namespace lsp
